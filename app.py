...
def save_network_to_cache(G, cache_file):
...
def load_network_from_cache(cache_file):
...
def get_street_network(place, network_type):
    ...
    cache_file = f'network_cache_{network_type}.pkl'
    
    # Prova a caricare dalla cache
    G = load_network_from_cache(cache_file)
    ...
    if G is None:
        try:
            with st.spinner('Scaricamento della rete stradale...'):
                ...
                save_network_to_cache(G, cache_file)
        except Exception as e:
...
@st.cache_data
def calculate_connectivity_scores(quartieri, G, poi, max_distance):
